...
static inline bool IsRegisterHFAParameter(const asCDataType &type, const asQWORD numFloatRegArgs)
{
	...
	// Overgrowth ARM64 patch: GetSizeInMemoryDWords() reports 32-bit words,
	// while the divisor below is in bytes. The unit mismatch can admit a
	// ninth HFA register and make CallARM64's computed branch loop forever.
	const int registersUsed = type.GetSizeInMemoryBytes() / (doubles ? sizeof(double) : sizeof(float));
