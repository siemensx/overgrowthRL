...
void UpdateMultiplayer(int frames)
...
void SetRemoteAirWhoosh(int id, float volume, float pitch);
...
class ModLevel {
    ...
    string GetID();
    string GetThumbnail();
    bool GetSupportsOnline();
    bool GetRequiresOnline();
    string GetPath();
...
bool ModGetSupportsOnline(ModID& id);
bool ModGetSupportsCurrentVersion(ModID& id);
...
bool IsExternalRLController(int controller_id);
bool IsMatchOverlayVisible();
...
class Level {
    ...
    void SetIntVar(const string &in name, int value);
    void SetArrayIntVar(const string &in name, int index, int value);
    void SetFloatVar(const string &in name, float value);
    void SetBoolVar(const string &in name, bool value);
...
void Online_Connect(string ip);
void Online_Host();
void Online_SendKillMessage(uint victim, uint killer);
void Online_SendChatMessage(string message);
void Online_SendChatSystemMessage(string system_message);
bool Online_IsActive();
bool Online_IsClient(void);
bool Online_IsHosting(void);
void Online_SetAvatarCameraAttachedMode(bool mode);
bool Online_IsAvatarCameraAttached();
void Online_SendStateToPeer(uint state, uint avatar_id, const array<uint>& data);
uint Online_RegisterState(string state);
uint Online_RegisterStateCallback(uint state, string callback);
void Online_SendState(uint state, const array<uint8>& data);
void Online_AddSyncState(uint state, const array<uint8>& data);
bool Online_IsValidPlayerName(string name);
void Online_Close();
bool Online_IsLobbyFull();
bool Online_HasActiveIncompatibleMods();
string Online_GetActiveIncompatibleModsString();
bool Online_HasFriendInviteOverlay();
void Online_ActivateInviteDialog();
enum CollisionSides {
    DOUBLE_SIDED = 1,
    SINGLE_SIDED = 0,
};
...
class MovementObject {
    ...
    void SetIntVar(string var_name, int value);
    void SetArrayIntVar(const string &in var_name, int index, int value);
    void SetFloatVar(string var_name, float value);
    void SetBoolVar(string var_name, bool value);
    ...
    void UpdateWeapons();
    void CDisplayMatrixUpdate();
    vec3 CGetCenterOfMassEstimate(array<BoneTransform> &in key_transforms, const array<float> &in key_masses, const array<int> &in root_bone);
    void CDoChestIK(float chest_tilt_offset, float angle_threshold, float torso_damping, float torso_stiffness, int num_frames);
    void CDoHeadIK(float head_tilt_offset, float angle_threshold, float head_damping, float head_accel_inertia, float head_accel_damping, float head_stiffness, int num_frames);
    void CDoFootIK(const BoneTransform &in local_to_world, int num_frames);
    void CDoHipIK(BoneTransform &inout hip_offset, quaternion &inout hip_rotate, int num_frames);
    void CDrawLeg(bool right, const BoneTransform &in hip_transform, const BoneTransform &in foot_transform, int num_frames);
    ...
    void CDrawHead(const BoneTransform &in chest_transform, const BoneTransform &in head_transform, int num_frames);
    void CDrawBody(const BoneTransform &in hip_transform, const BoneTransform &in chest_transform);
    void CDrawEar(bool right, const BoneTransform &in head_transform, int num_frames);
    void CDrawTail(int num_frames);
    void CDrawFinalBoneIK(int num_frames);
    void CUpdateShadow();
    void CUpdateEyeLook();
