@echo off
REM OGRL-20260924-010 run27_win: run26's recipe continued from its 270M checkpoint on the
REM engine build with both 2026-09-24 actuator fixes (BuildWinFix_20260924):
REM   * rl_button_edges: 1 -- a held button is ONE press, as for a human. Before, every held
REM     physics tick was a fresh press: holding grab gave one active block per hold and then a
REM     lockout (probe: 1 block in 30 press cycles; with the fix 30 in 30).
REM   * the observed "self" is the player-controlled character, not the first character on
REM     controller 0 (only mattered in 1v1; 1v3 probes on maps 101/102/104 were already correct).
REM Everything else is run26 (below). No critic reset: same gamma/reward.
REM ---- run26 header follows for provenance ----
...
2>nul ( 9>C:\ogrl\run_forever.lock ( call :main %* ) ) || (
  echo [%date% %time%] run27: another supervisor holds the lock - exiting >> C:\ogrl\run27.log
  exit /b 1
)
exit /b 0
...
set RUN=run27_win
set BASE=%REPO%\Tools\rl\ppo\checkpoints\run26_win_final.pt
...
if exist "%CKPT%" (
  ...
  set START=--resume-from %BASE% --reset-running-return
)
echo [%date% %time%] %RUN%: launch %TRIES% %START% >> C:\ogrl\run27.log
%PY% -u Tools\rl\ppo\train_vec.py ^
  --repo-root %REPO% ^
  --levels arenas/t_train_101.xml,arenas/t_train_102.xml,arenas/t_train_104.xml ^
  --shm-prefix /ogrl_r27_%RANDOM%%TRIES% --n-envs 20 --k-standby 4 --seed 27 --allow-n-envs-change ^
  ...
  --purpose "OGRL-20260924-010 run26 + native button edges + player-identity fix" >> C:\ogrl\%RUN%.out 2>&1
set RC=%ERRORLEVEL%
echo [%date% %time%] %RUN%: exited %RC% >> C:\ogrl\run27.log
