@echo off
REM Phase 1 of OPTIMIZATION_CONTRACT.md: actuator A/B. Sequential arms, each resumed
...
2>nul ( 9>C:\ogrl\run_forever.lock ( call :main %* ) ) || (
  echo [%date% %time%] phase1: another supervisor holds the lock - exiting >> C:\ogrl\phase1.log
  exit /b 1
)
exit /b 0
...
set LOG=C:\ogrl\phase1.log
if "%ARMS%"=="" set ARMS=0 2
cd /d %REPO%
echo [%date% %time%] phase1 start arms=%ARMS% >> %LOG%
for %%A in (%ARMS%) do call :arm %%A
echo [%date% %time%] phase1 done >> %LOG%
exit /b 0

:arm
set SEL=%1
set RUN=run22_sel%SEL%
...
%PY% -u Tools\rl\ppo\train_vec.py ^
  ...
  --shm-prefix /ogrl_p1%SEL%%RANDOM% --n-envs 14 --k-standby 4 --seed 22 ^
  ...
  --periodic-eval-steps 5000000 --periodic-eval-episodes 200 ^
